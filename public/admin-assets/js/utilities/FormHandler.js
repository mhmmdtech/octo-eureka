...
class FormHandler {
  ...
  isStrongPassword(input) {
    const password = input.value.trim();
    const regex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{5,30}$/;
...
class FormValidation extends FormHandler {
  ...
  attachmentValidation(input, subject, errorWrapperName, isRequired) {
    let allowedSize = 20 * 1024 * 1024;
    if (isRequired && this.isEmpty(input)) {
      this.addError(input, errorWrapperName, `${subject} Is Required`);
      return false;
    }
    if (!this.isEmpty(input)) {
      ...
      for (const [index, file] of Object.entries(attachments)) {
        ...
        if (this.isGreaterThanMaxSize(fileSize, allowedSize)) {
          this.addError(
            input,
            errorWrapperName,
            `One ${subject} is greather than 20MB`
          );
        }
      }
    }
