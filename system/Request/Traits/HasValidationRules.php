...
trait HasValidationRules
{
    ...
    {
        // it was changed, we remobe => for logic
    ...
    protected function password($name)
    {
        ...
        if ($this->checkFieldExist($name)) {
            if (!preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{5,30}$/", $this->request[$name]) && $this->checkFirstError($name)) {
                $this->setError($name, "$name must be stronger");
            }
        }
    }
